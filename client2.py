    ...
    def datagramReceived(self, datagram, addr):
        if addr == self.server:
            ...
            nowy_blok = Block(blockchain[-1]['index'], blockchain[-1]['hash'],
                              ...
                              TRAN_1.to_json(), blockchain[-1]['nonce'], blockchain[-1]['difficulty'])
            print(type(nowy_blok))
            if flag == 0:
                if nowy_blok.index != blockchain2[-1].index:
                    blockchain2.append(nowy_blok)

                if int(nowy_blok.index) > int(blockchain2[-2].index)+1:
                    # print('Gę', nowy_blok.index, int(blockchain2[-2].index)+1)
                    kopia = blockchain2.copy()
                    self.transport.write(f"Wyślij mi bloki od nr {int(blockchain2[-2].index)+1} do numeru {nowy_blok.index-1}".encode("utf-8"), ('127.0.0.1', 3000))
            else:
                for ele in blockchain2:
                    print('tututu',int(nowy_blok.index-1),int(ele.index), int(nowy_blok.index-1) - int(ele.index))
                    if int(nowy_blok.index-1) - int(ele.index) == 1:
                        blockchain2.insert(int(nowy_blok.index), nowy_blok)
                        bzyt = blockchain2[nowy_blok.index].data
                        if type(bzyt) == str:
                            json_string_fixed = bzyt.replace("'", "\"")
                            bzyt = json.loads(json_string_fixed)
                        transakcja = Transaction([TxIn(bzyt['tx_ins'][0]['tx_out_id'], 0)],
                                                 [TxOut(bzyt['tx_outs'][0]['address'], 100)])
                        print(transakcja)

                        if zweryfikuj_transakcje2(transakcja,utxo_set):
                            break
                        else:
                            print('Transakcja niezweryfikowana, wracamy do poprzedniego łańcucha')
                            blockchain2.pop()
                            blockchain2.pop()
                            blockchain2.pop()
                            break
...
def zweryfikuj_transakcje(T, podpis):
    for i in range(len(utxo_set)):
        ...
        if utxo_set[i]['txOutId'] == T.tx_ins[0].tx_out_id and utxo_set[i]['txOutIndex'] == T.tx_ins[0].tx_out_index:
            ...
            print('     Transakcji nie ma w UTXO')
...
print(TRAN_1.to_json())
# zweryfikuj_transakcje(TRAN_1, podpis_1)
...
# zweryfikuj_transakcje(TRAN_2, podpis_2)
