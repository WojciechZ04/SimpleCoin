...
import hashlib
import block
blok = block.Block('Data', hashlib.sha256())
import time
    ...
    def send_message(self):
        while True:
            ...
            if msg.startswith("[Add nodes]") == False:
                ...
                enc_msg = rsa.encrypt(blok.to_json_prev_hash().encode("utf-8"), kl_pub)
                ...
                enc_msg = rsa.encrypt(blok.to_json_nonce().encode("utf-8"), kl_pub)
                signature = rsa.sign(enc_msg, self.priv_key, "SHA-256")
                print(f"Port podpisującego/wysyłającego: {self.id[1]}")
                with open(f"{self.id[1]}signature", "wb") as f:
                    f.write(signature)
                ...
                self.transport.write(enc_msg, ('127.0.0.1', port))
                time.sleep(2)
                enc_msg = rsa.encrypt(blok.to_json_data().encode("utf-8"), kl_pub)
                signature = rsa.sign(enc_msg, self.priv_key, "SHA-256")
                print(f"Port podpisującego/wysyłającego: {self.id[1]}")
                with open(f"{self.id[1]}signature", "wb") as f:
                    f.write(signature)
