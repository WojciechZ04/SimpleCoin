...
class Client(DatagramProtocol):
    ...
    def datagramReceived(self, datagram, addr):
        if addr == self.server:
            ...
            print(addr, ":", datagram)
            print(datagram)
            if datagram.startswith("Wyślij") == False and datagram.startswith("Przestań") == False:
                ...
                if int(nowy_blok.index) > int(blockchain2[-1].index)+1:
                    print('Wiekszy')
    ...
    def send_message(self):
        while True:
            ...
            if msg.startswith("[Add nodes]") == False and msg.startswith("[Add block]") == False and msg.startswith(
                    "[Send block]") == False:
            ...
            if msg.startswith("[Add block]"):
                ...
                port = int(input('Podaj port wezla'))
                self.transport.write(msg.encode("utf-8"), ('127.0.0.1', port))
            if msg.startswith("[Send block]"):
                numer_bloku = int(input('Podaj numer bloku'))
                #dla drugiej kropki zmienić poniżej "-2" na "numer_bloku"
                blok_do_wyslania ='$' + str(blockchain2[numer_bloku].to_json())
                port = int(input('Podaj port wezla'))
                self.transport.write(blok_do_wyslania.encode("utf-8"), ('127.0.0.1', port))
